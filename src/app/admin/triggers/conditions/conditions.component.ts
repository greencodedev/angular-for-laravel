...
export class ConditionsComponent {
    ...
    public removeCondition(condition: {condition: Condition | number}) {
        const conditions = this.model.filter(condition => condition['matchType'] === this.matchType);

        const index = this.model.findIndex((conditionModel: {condition: Condition | number}) => {
